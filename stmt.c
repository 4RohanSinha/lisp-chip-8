...
static int operators[] = {T_PLUS, T_MINUS, T_STAR, T_SLASH, T_SETQ, T_LET, T_IDENT, T_CLS, T_IF, T_SPRITE_LITERAL_DECL};
...
static struct ast_node *process_paren()
{
	...
	while (t.tokentype != T_CLOSE_PAREN)
	{
		...
		case T_IDENT:
			new_child = (struct ast_node *)malloc(sizeof(struct ast_node));
			new_child->key.sloc = t.val.sloc;
			new_child->t_operatortype = t.tokentype;
			new_child->kChildren = 0;
			cur_node->children[(cur_node->kChildren)++] = new_child;
			break;
		default:
...
static void print_ast(struct ast_node *tree, int indent)
{
	for (int i = 0; i < indent; i++)
		printf("\t");
	if (tree->t_operatortype == T_INTLIT)
		printf("%d ", tree->key.val);
	else if (tree->t_operatortype == T_IDENT)
		printf("%s ", sym_get_symbol_for(tree->key.sloc));
